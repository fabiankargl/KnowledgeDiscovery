...
def extract_birth_info(birthday_str):
    if pd.isna(birthday_str):
        return pd.Series([None, None, None, None],
                         index=["Birthday", "Age", "Birth City", "Birth Country"])
    
    birthday_str = clean_text(birthday_str)

    date_match = re.search(r"([A-Za-z]+\s+\d{1,2},\s*\d{4})", birthday_str)
    birthday = None
    if date_match:
        try:
    ...
    age_match = re.search(r"Age:\s*([0-9\-]+)", birthday_str)
    age = None
    if age_match:
        age = age_match.group(1).split("-")[0].strip()


    place_match = re.search(r"in\s*([\w\s\.-]+?),\s*([\w\s]+?)(?:\s+([A-Za-z]{2}))?$", birthday_str)
...
def clean_player_data(df):
    ...
    if "Birthday" in df.columns:
        birthday_parts = df["Birthday"].progress_apply(extract_birth_info)
        df = pd.concat([df, birthday_parts], axis=1)
